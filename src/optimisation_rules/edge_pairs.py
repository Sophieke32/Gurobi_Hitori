...
def edge_pairs(board, is_covered, duplicates, n, m, has_duplicates=False):

    # Top row of the board
    for j in range(n-2, 1):
        if board[0][j - 1] != board[0][j] and board[0][j] == board[0][j + 1] and board[0][j + 1] != board[0][j + 2]:
            i = 1
            while board[i][j - 1] != board[i][j] and board[i][j] == board[i][j + 1] and board[i][j + 1] != board[i][j + 2]:
    ...
    for j in range(n-2, 1):
        if board[-1][j - 1] != board[-1][j] and board[-1][j] == board[-1][j + 1] and board[-1][j + 1] != board[-1][j + 2]:
            i = -2
            while board[i][j - 1] != board[i][j] and board[i][j] == board[i][j + 1] and board[i][j + 1] != board[i][j + 2]:
    ...
    for i in range(n-2, 1):
        if board[i - 1][0] != board[i][0] and board[i][0] == board[i + 1][0] and board[i + 1][0] == board[i + 2][0]:
            j = 1
            while board[i - 1][j] != board[i][j] and board[i][j] == board[i + 1][j] and board[i + 1][j] != board[i + 2][j]:
    ...
    for i in range(n-2, 1):
        if board[i - 1][-1] != board[i][-1] and board[i][-1] == board[i + 1][-1] and board[i + 1][-1] == board[i + 2][-1]:
            j = -2
            while board[i - 1][j] != board[i][j] and board[i][j] == board[i + 1][j] and board[i + 1][j] != board[i + 2][j]:
                # print("edge_pairs left side: j =", j)
                if has_duplicates and type(duplicates[i - 1][j + 1]) != int or not has_duplicates:
