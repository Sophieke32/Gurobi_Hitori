...
# It then adds a constraint that n-1 out of n of those values need to be covered
def duplicates_unique_constraint(n, is_covered, m, duplicates):
    ...
    for i in range(n):
        for j in range(n):
            if duplicates[i][j] > 0:
                horizontal_expressions[i][int(duplicates[i][j]) -1].append(1 - is_covered[i][j])
    ...
    for i in range(n):
        for j in range(n):
            if duplicates[j][i] > 0:
                vertical_expressions[i][int(duplicates[j][i]) - 1].append(1 - is_covered[j][i])
